...
Also create a minimal ONNX model fixture for tests
(`tests/fixtures/models/simple_predictor.onnx`) generated by a
script in `tests/generate_model_fixtures.py`. Use a tiny model
(linear regression with 3 features) so the fixture is small and
loads instantly.
